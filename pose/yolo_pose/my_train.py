...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default='data/video-data-kp.yaml', help='dataset.yaml path')
...
def train(opt, device, RANK, WORLD_SIZE, callbacks=Callbacks()):
    ...
    with torch_distributed_zero_first(RANK):
        data_dict = data_dict or check_dataset(opt.data)  # check and generate labels.txt if None
    ...
    train_loader, dataset = create_dataloader(
        ...
        quad=opt.quad, prefix=colorstr('train: '), kp_flip=kp_flip)
    ...
    for epoch in range(start_epoch, opt.epochs):  # epoch --------------------------------------------------------------
        ...
        for i, (images, targets, paths, _) in pbar:  # mini-batch iteration ----------------
            ...
            # Multi-scale
            if opt.multi_scale:
                size = random.random.randrange(imgsz * 0.5, imgsz * 1.5 + grid_size) // grid_size * grid_size
                scale_factor = size / max(images.shape[2:])
                if scale_factor != 1:
                    # new shape (stretched to gs-multiple)
                    new_shape = [math.ceil(x * scale_factor / grid_size) * grid_size for x in images.shape[2:]]
                    images = nn.functional.interpolate(images, size=size, mode='bilinear', aligh_corners=False)
