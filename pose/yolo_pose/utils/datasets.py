...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, labels_dir='labels', img_size=640, batch_size=16, augment=False, hyp=None, rect=False,
        ...
        # 2.Get a path list of image files  => self.img_files
        try:
            f = []  # image files
        ...
        # 6. Rectangular Training (Disable)
        if self.rect:
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        with Pool(NUM_THREADS) as pool:
            ...
            for im_file, l, shape, segments, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                ...
                nc += nc_f
                if im_file:
                    x[im_file] = [l, shape, segments]  # [np.zeros((5 + num_joints*3)), (w, h), []]
                if msg:
                    msgs.append(msg)
    ...
    def __getitem__(self, index):
        ...
        # labels_out => [img_idx, bbox_cls_id, xc, yc, w, h, x1,y1,v1, ..., xk,yk,vk]
...
def autosplit(path='../data/coco128/images', weights=(0.9, 0.1, 0.0), annotated_only=False,
