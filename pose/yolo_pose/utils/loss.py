...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False, num_coords=0, device=None, joint_weights=None):
        ...
        # Define criteria
        self.joint_weights = torch.tensor(joint_weights, device=device) if joint_weights != None else None
        if self.joint_weights is None:
            BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device))
        else:
            BCEcls = nn.BCEWithLogitsLoss(pos_weight=self.joint_weights)
    ...
    def __call__(self, predict, targets):  # predictions, targets, model
        device = targets.device
        lcls = torch.zeros(1, device=device)
        lbox = torch.zeros(1, device=device)
        lobj = torch.zeros(1, device=device)
        lkps = torch.zeros(1, device=device)  # keypoint loss
        tcls, tbox, t_kpts, indices, anchors = self.build_targets(predict, targets)  # targets

        # Losses
        for i, pi in enumerate(predict):
            # layer index, layer predictions ([batch, 3, 80, 80, 5+1+num_joints*3])
            ...
            if n:
                ...
                out_positive = pi[img_id, anchor_id, gj, gi]  # prediction subset corresponding to targets

                # Regression
                pxy = out_positive[:, :2].sigmoid() * 2. - 0.5  # -0.5~1.5, ignore the grid cell coordinates (cx, cy)
                pwh = (out_positive[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]  # range [0, 4] * anchor
                ...
                if self.num_coords:
                    t_kpt = t_kpts[i]
                    vis = t_kpt[..., 2] > 0.2  # default is > 0, but we add pseudo keypoints from MediaPipe, whose vis is 0~1
                    kpt_vis = t_kpt[vis]
                    if len(kpt_vis):
                        pkp = out_positive[:, 5 + self.nc:].reshape((-1, self.num_coords // 2, 2))
                        ...
                        if self.joint_weights != None:
                            joint_weights = self.joint_weights.tile((vis.shape[0], 1))
                            l2 *= joint_weights[vis]
                        lkps += torch.mean(l2)

                # Objectness            => bbox confidence
                ...
                # Classification       => keypoints confidence
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(out_positive[:, 5:5 + self.nc], self.cn, device=device)  # targets
                    t[vis] = 1
                    lcls += self.BCEcls(out_positive[:, 5:5 + self.nc], t)  # BCE
    ...
    def build_targets(self, pred, targets):
        ...
        for i in range(self.nl):
            ...
            # if grid_size = 80, gain =
            # [1, 1,                                   # img_id, cls_id
            #  80, 80, 80, 80,                         # xc, yc, w, h
            #  80, 80, 1, 80, 80, 1, ..., 80, 80, 1,   # x1, y1, v1, x2, y2, v2, ..., xk, yk, vk
            #  1]                                      # anchor_id
