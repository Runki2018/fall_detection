...
def arg_parser():
    ...
    parser.add_argument('--vis-thres', type=float, default=0.3, help='visible keypoint threshold')
...
if __name__ == '__main__':
    ...
    for i, (path, img, img0, _) in enumerate(dataset):
        ...
        bboxes, poses, _, _, _ = post_process_batch(data, img, [], [[img0.shape[:2]]], person_dets)
        ...
        for j, (bbox, pose) in enumerate(zip(bboxes, poses)):
            x1, y1, x2, y2 = bbox
            cv2.rectangle(img0_copy, (int(x1), int(y1)), (int(x2), int(y2)), args.color, thickness=3)
            if args.csv:
                for x, y, c in pose:
            ...
            for x,y,c in pose:
                ...
                if c < args.vis_thres: # keypoints with human bbox
