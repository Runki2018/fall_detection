...
def run_nms(data, model_out):
    if data['iou_thres'] == data['iou_thres_kp'] and data['conf_thres_kp'] >= data['conf_thres']:
        ...
        dets = non_max_suppression_kp(model_out, data['conf_thres'],
                                      data['iou_thres'],
                                      num_coords=data['num_coords'],
                                      swap_nms=data.get('swap_nms', False))
        person_dets = [d[d[:, 5] == 0] for d in dets]
        kp_dets = [d[d[:, 4] >= data['conf_thres_kp']] for d in dets]  # filter by bbox confidence
        kp_dets = [d[d[:, 5] > 0] for d in kp_dets]  # keep keypoint bbox
    else:
        ...
        kp_dets = non_max_suppression_kp(model_out, data['conf_thres_kp'], data['iou_thres_kp'],
                                         classes=list(range(1, 1 + data['num_coords'] // 2)),
...
def post_process_batch(data, imgs, paths, shapes, person_dets, kp_dets,
                       two_stage=False, pad=0, device='cpu', model=None, origins=None, path2img_id=None):

    batch_bboxes, batch_poses, batch_scores, batch_ids = [], [], [], []
    num_joints = data['num_coords'] // 2
    n_fused = np.zeros(num_joints)
    ...
    for si, (pd, kpd, origin) in enumerate(zip(person_dets, kp_dets, origins)):
        ...
        if num_det:
            ...
            if path2img_id is None:
                ...
                #  but not work for mixed-pose dataset
            ...
            if two_stage:
                ...
                for bbox in pd[:, :4].cpu().numpy():
                    ...
                    if len(poses):
                        ...
                        poses = poses.reshape(poses.shape[0], num_joints, 2)
                        poses = np.concatenate((poses, np.zeros((poses.shape[0], num_joints, 1))), axis=-1)
            ...
            # SINGLE-STAGE INFERENCE
                ...
                poses_xy = scale_coords(imgs[si].shape[1:], pd[:, -num_joints*2:], shape).cpu().numpy()
                poses_xy = poses_xy.reshape((num_det, num_joints, 2))  # [num_det, num_kp, 2]
                poses_vis = pd[:, -num_joints*3:-num_joints*2].cpu().numpy().reshape((num_det, num_joints, 1))
                # the confidence of keypoints which are predict with human bbox is default to 1.0
                poses = np.concatenate([poses_xy, poses_vis], axis=-1)  # [num_det, num_kp, 3]

                # fuse kpt bbox to keypoint
                if data['use_kp_dets'] and num_det_kpt:
                    mask = scores > data['conf_thres_kp_person']
                    poses_mask = poses[mask]
                    bboxes_mask = bboxes[mask]
                    # auto_overwrite_tol = (bboxes_mask[:, 2] - bboxes_mask[:, 0]), bboxes_mask[:, 2] - bboxes_mask[:, 0]

                    if len(poses_mask):
                        kpd[:, :4] = scale_coords(imgs[si].shape[1:], kpd[:, :4], shape)
                        kpd = kpd[:, :6].cpu()
                        xc = kpd[:, 0:4:2].mean(axis=1, keepdims=True)
                        yc = kpd[:, 1:4:2].mean(axis=1, keepdims=True)
                        kpd_xyci = np.concatenate([xc, yc, kpd[:, 4:5], kpd[:, 5:6]], axis=1)
                        # TODO: change the loop of the subject from kpd to pose_mask, to speed the inference up
                        for bbox, pose in zip(bboxes_mask, poses_mask):
                            bx1, by1, bx2, by2 = bbox[:4]
                            max_side =  max(bx2-bx1, by2-by1)
                            mean_pose_conf = pose[:, 2].mean() + 10

                            for kp_id, (xyc, ow_ratio) in enumerate(zip(pose, data['ow_ratios']), start=1):
                                x, y, c = xyc
                                auto_overwrite_tol = ow_ratio * max_side
                                kp_bbox_center = kpd_xyci[(kpd_xyci[:, 3] == kp_id) & (kpd_xyci[:, 3] > 0.2)]
                                if kp_bbox_center.size == 0:
                                    continue

                                dist = np.linalg.norm(np.array([[x, y]]) - kp_bbox_center[:, :2], axis=-1)
                                kp_match = np.argmin(dist)
                                x, y, c = kp_bbox_center[kp_match, :3]
                                if c < mean_pose_conf and dist[kp_match] < auto_overwrite_tol \
                                        and bx1 <= x <= bx2 and by1 <= y <= by2:  # within the bbox
                                    # pose[kp_id-1][:2] = [x, y]
                                    pose[kp_id-1] = [x, y, c]
                                    if data['count_fused']:
                                        n_fused[kp_id-1] += 1
                        ...
                        #     x, y = np.mean((x1, x2)), np.mean((y1, y2))  # keypoint xy is the center of keypoint bbox
                        #     pose_kps = poses_mask[:, int(cls - 1)]
                        #     dist = np.linalg.norm(pose_kps[:, :2] - np.array([[x, y]]), axis=-1)
                        #     kp_match = np.argmin(dist)
                        #     # TODO: why conf > pose_kps[kp_match, 2], and the later always equal 0, Do here ignore something?
                        #     if dist[kp_match] < data['overwrite_tol']:
...
def run(data,
        ...
        json_name='',
        swap_nms=False,
        ow_ratios=(0.1, 0.1, 0.15, 0.15, 0.2, 0.2, 0.1, 0.1, 0.2, 0.2, 0.3, 0.3),   # overwrite_tol = ow_ratio * max(bbox_w, bbox_h)
        ):
    ...
    data['swap_nms'] = swap_nms
    data['ow_ratios'] = ow_ratios
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc='Processing {} images'.format(task))):
        ...
        _, poses, scores, img_ids, n_fused_batch = post_process_batch(
            data, imgs, paths, shapes, person_dets, kp_dets,
...
def parse_opt():
    ...
    parser.add_argument('--swap_nms', action='store_true', help="NMS the left-right keypoints pair")
