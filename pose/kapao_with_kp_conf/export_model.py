...
def export_onnx(opt):
    ...
    model = Model(ckpt['model'].yaml, ch=3,
                  nc=nc, anchors=hyp.get('anchors'), num_coords=num_coords)
    csd = ckpt['model'].float().state_dict()  # checkpoint state_dict as FP32
    model.load_state_dict(csd, strict=False)
    model.eval()
    input_img = torch.rand((1, 3, opt.imgsz, opt.imgsz))
    with torch.no_grad():
        model_ts = model.float()
        model_ts = torch.jit.trace(model_ts, input_img)
        # model = torch.jit.script(model)
        # model = model.to(device)
        torch_script_file = Path(opt.weights).with_name("torch_script.pt")
        torch.jit.save(model_ts, torch_script_file)
        print(f"{torch_script_file=}")
    ...
    # input_img = torch.rand((1, 3, opt.imgsz, opt.imgsz), device=device)

    # 3. Export ONNX
    torch.onnx.export(
        model.cpu() if opt.dynamic else model,   # --dynamic only compatible with cpu
        input_img.cpu() if opt.dynamic else input_img,
        ...
        dynamic_axes=opt.dynamic or None)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='runs/pose12_v7tiny_e100_512/with_conf2/weights/best.pt',
