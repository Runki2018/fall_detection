...
try:
    ...
    from pose.kapao_with_kp_conf.utils.general import colorstr, segment2box, resample_segments, check_version
    from pose.kapao_with_kp_conf.utils.metrics import bbox_ioa
...
def random_perspective(im, targets=(), segments=(), degrees=10, translate=.1, scale=.1,
                       shear=10, perspective=0.0, border=(0, 0), kp_bbox=None, prob_rotate90=0.0):
    ...
    if random.uniform(0, 1) < prob_rotate90:
        a += random.choice([-180, -90, -90, 90, 90])  # add 90deg rotations to small rotations
