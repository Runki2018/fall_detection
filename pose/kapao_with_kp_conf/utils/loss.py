...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False, num_coords=0,
        ...
        cls_weights = torch.tensor([1] + joint_weights, device=device) if joint_weights != None else None
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=device))
        # BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device))
        BCEcls = nn.BCEWithLogitsLoss(weight=cls_weights, pos_weight=torch.tensor([h['cls_pw']], device=device))
        BCEvis = nn.BCEWithLogitsLoss(weight=self.joint_weights)
        ...
        if g > 0:
            BCEcls, BCEobj, BCEvis = FocalLoss(BCEcls, g), FocalLoss(BCEobj, g), FocalLoss(BCEvis, g)
        ...
        for i, pi in enumerate(predict):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.num_coords:
                    ...
                    kpt_vis = t_kpt[vis]
                    if len(kpt_vis):
                        num_joints = self.num_coords // 2
                        pkp_xy = ps[:, 5 + self.nc + num_joints:].reshape((-1, num_joints, 2))  # [x, y]
                        pkp_xy = (pkp_xy.sigmoid() * 4. - 2.) * anchors[i][:, None, :]  # range [-2, 2] * anchor
                        pkp_vis = pkp_xy[vis]
                        ...
                        lkps += torch.mean(l2)
                        # vis_conf loss
                        vis_conf = ps[:, 5 + self.nc:5 + self.nc + num_joints]
                        t = torch.full_like(vis_conf, self.cn, device=device)  # targets
                        t[vis] = self.cp  # 1.0 smooth positive
                        lkps += self.BCEvis(vis_conf, t)  # BCE TODO: loss weights between kpt and vis

                # Objectness
                score_iou = iou.detach().clamp(0).type(t_obj.dtype)
                if self.sort_obj_iou:
                    sort_id = torch.argsort(score_iou)
                    img_id, anchor_id, gj, gi, score_iou = img_id[sort_id], anchor_id[sort_id],\
                                                           gj[sort_id], gi[sort_id], score_iou[sort_id]
