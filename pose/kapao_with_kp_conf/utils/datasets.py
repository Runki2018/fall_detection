...
try:
    ...
    from pose.kapao_with_kp_conf.utils.augmentations import Albumentations, augment_hsv, copy_paste, letterbox, mixup, random_perspective, cutout
    from pose.kapao_with_kp_conf.utils.general import check_requirements, check_file, check_dataset, xywh2xyxy, xywhn2xyxy, xyxy2xywhn, \
        xyn2xy, segments2boxes, clean_str
    from pose.kapao_with_kp_conf.utils.torch_utils import torch_distributed_zero_first
...
def create_dataloader(path, labels_dir, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False,
                      cache=False, pad=0.0, rect=False, rank=-1, workers=8, image_weights=False, quad=False,
                      prefix='', kp_flip=None, kp_bbox=None):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, labels_dir='labels', img_size=640, batch_size=16, augment=False, hyp=None, rect=False,
    ...
    def __getitem__(self, index):
                ...
                if nl:
                    ...
                    if self.obj_flip:
                        for i, cls in enumerate(labels[:, 0]):
                            labels[i, 0] = self.obj_flip[labels[i, 0]]  # change class id correspondingly

            # Cutouts
            labels = cutout(img, labels, p=hyp['cutout'])

        # visualization for augment debug
        ...
        if self.augment and hyp['cutout'] > 0:
            labels_out = torch.zeros((labels.shape[0], labels.shape[-1] + 1))
        else:
            labels_out = torch.zeros((nl, labels.shape[-1] + 1))  # error when enable cutout
        if nl:
...
def load_mosaic(self, index, kp_bbox=None):
    ...
    img4, labels4 = random_perspective(img4, labels4, segments4,
                                       ...
                                       kp_bbox=kp_bbox,
                                       prob_rotate90=self.hyp.get('prob_rotate90', 0.0),
                                       )  # border to remove
    return img4, labels4
...
def flatten_recursive(path='../data/coco128'):
...
def extract_boxes(path='../data/coco128', labels_dir='labels'):  # from utils.data import *; extract_boxes()
...
def autosplit(path='../data/coco128/images', weights=(0.9, 0.1, 0.0), annotated_only=False, labels_dir='labels_dir'):
    """ Autosplit a dataset into train/val/test splits and save path/autosplit_*.txt files
    Usage: from utils.data import *; autosplit()
    Arguments
...
def dataset_stats(path='coco128.yaml', autodownload=False, verbose=False, profile=False, hub=False, labels_dir='labels'):
    """ Return dataset statistics dictionary with images and instances counts per split per class
        ...
        Usage1: from utils.data import *; dataset_stats('coco128.yaml', autodownload=True)
        Usage2: from utils.data import *; dataset_stats('../data/coco128_with_yaml.zip')
        Arguments
