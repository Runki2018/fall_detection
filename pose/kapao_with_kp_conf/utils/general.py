...
def check_dataset(data, autodownload=True):
    ...
    if isinstance(data, (str, Path)) and str(data).endswith('.zip'):  # i.e. gs://bucket/dir/coco128.zip
        download(data, dir='../data', unzip=True, delete=False, curl=False, threads=1)
        data = next((Path('../data') / Path(data).stem).rglob('*.yaml'))
...
def non_max_suppression_kp(prediction, conf_thres=0.25, iou_thres=0.45,
                           classes=None, max_det=300, num_coords=24, swap_nms=False):
    """Runs Non-Maximum Suppression (NMS) on inference results
    prediction: (tensor)[batch, num_anchor_points, 57] = [B, 25200, 50], when num_head == 3, imgz==640
    57: [x,y,w,h,conf, person, c1,c2,..., c17, x1,y1,..., x17, y17]
    swap_nms: if ture, NMS the left keypoints and right keypoints
    Returns:
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        # x: [x,y,w,h,bbox_conf, person_conf, c1, c2, c3, c4,...,c17, x1,y1, x2,y2, ..., x17, y17]
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]  # confidence
        ...
        # Compute conf [batch, 13]
        x[:, 5:-num_joints*3] *= x[:, 4:5]  # conf = obj_conf * cls_conf

        # Box (center x, center y, width, height) to (x1, y1, x2, y2)
        box = xywh2xyxy(x[:, :4])

        # Detections matrix nx6 (xyxy, conf, cls)
        class_conf, class_id = x[:, 5:-num_joints*3].max(1, keepdim=True)
        kpt_xyv = x[:, -num_joints*3:]
        # x: [x, y, w, h, bbox_conf, class_idx, v1, v2, .., vk, x1, x2, ..., xk, yk]
        x = torch.cat((box, class_conf, class_id.float(), kpt_xyv), 1)[class_conf.view(-1) > conf_thres]
        ...
        # if swap_nms:
        #     cls_id = x[:, 5:6].clone()
        #     cls_id[(cls_id > 0) & (cls_id % 2==0)] -= 1   # [0, 1,2,3,4, ..., 11, 12] -> [0,1,1,1,3,3,...,11,11]
        #     c =  cls_id * max_wh
        # else:
        #     c = x[:, 5:6] * max_wh  # classes as an offset to NMS different class bbox
        c = x[:, 5:6] * max_wh  # classes as an offset to NMS different class bbox
        ...
        if swap_nms:
            ...
            c =  cls_id * max_wh
            boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class), scores
            i = torchvision.ops.nms(boxes, scores, iou_threshold=0.85)  # NMS
