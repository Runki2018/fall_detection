...
try:
    ...
    from pose.kapao_with_kp_conf.utils.torch_utils import select_device, time_sync
    from pose.kapao_with_kp_conf.utils.general import check_img_size, scale_coords
    from pose.kapao_with_kp_conf.utils.datasets import LoadImages
...
def arg_parser():
    ...
    # video options
    parser.add_argument('-p', '--video-path', default='/home/huangzhiyong/Project/fall_detection/videos/1.mp4', help='path to video file')
    ...
    # model options
    parser.add_argument('--data', type=str, default='/home/huangzhiyong/Project//fall_detection/pose/kapao_with_kp_conf/data/pose12-kp.yaml')
    # parser.add_argument('--data', type=str, default='data/coco-kp.yaml')
    parser.add_argument('--imgsz', type=int, default=512)
    parser.add_argument('--weights',
                        default='/home/huangzhiyong/Project//fall_detection/pose/kapao_with_kp_conf/runs/pose12_v7tiny_e100_512/fintune_mosaic001/weights/last.pt')
    ...
    parser.add_argument('--swap-nms', action='store_true', help='NMS each pair of keypoints, such as left-ankle and right-ankle.')
...
def get_data_setting(args):
    ...
    data['swap_nms'] = args.swap_nms
    data['ow_ratios'] = args.ow_ratios
    return data
...
def draw_kp_bbox_as_circle(args, img, img0, kp_bbox, kp_idx=12, thickness=2):
    bbox = kp_bbox[kp_bbox[:, 5] == kp_idx]
    best_two_bbox_idx = bbox[:, 4].argsort(axis=0)[-2:]
    bbox = bbox[best_two_bbox_idx]
    if bbox.numel() > 0:
        if args.no_kp_dets:
            bbox[:, :4] = scale_coords(img.shape[2:],  # N,C,H,W
                                        bbox[:, :4],
                                        img0.shape[:2])  # H, W, C
        for bbox in bbox:
            x1, y1, x2, y2, conf = bbox[:5]
            xc, yc = int((x1 + x2) // 2), int((y1 + y2) // 2)
            if args.vis_conf_values:
                cv2.putText(img0, "{:.0f}".format(conf * 100), (int(x2), int(y2)),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), thickness=thickness)
            cv2.circle(img0, (xc, yc), args.kp_size + 10, (0, 0, 255), args.kp_thick)
...
if __name__ == '__main__':
    ...
    out_path = '{}_{}_{}'.format(osp.splitext(video_path)[0], osp.splitext(weights_name)[0],
                                 args.device if args.device == 'cpu' else 'gpu'+str(args.device))
    print(f"=> {out_path=}")
    ...
    for i, (path, img, img0, _) in enumerate(dataset):
        ...
        out = model(img, augment=True, kp_flip=data['kp_flip'], scales=data['scales'], flips=data['flips'])[0]  # TODO: why augment is True ?
        ...
        for j, (bbox, pose) in enumerate(zip(bboxes, poses)):
            x1, y1, x2, y2 = bbox
            # auto size for drawing line and point
            if args.kp_thick < 0 or args.line_thick:
                area = (x2 - x1) * (y2 - y1)
                args.kp_size, args.kp_thick, args.line_thick = auto_thickness(area)

            cv2.rectangle(img0_copy, (int(x1), int(y1)), (int(x2), int(y2)),
                          args.color, thickness=args.line_thick)
            ...
            # show keypoints bbox
            if len(kp_dets) > j and args.kp_thick > 1:
                kp_bbox = kp_dets[j]
                for kp_idx in range(pose.shape[0]):   # num_joints
                    draw_kp_bbox_as_circle(args, img, img0_copy, kp_bbox, kp_idx=kp_idx, thickness=2)
            ...
            if args.kp_size > 0:
                for i, (x,y,c) in enumerate(pose, start=1):
                    if args.vis_conf_values and args.kp_thick > 1 and i in [5, 6, 11, 12]:
                        cv2.putText(img0_copy, "{:.0f}".format(c*100), (int(x+10), int(y+10)),
