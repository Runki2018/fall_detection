...
config = {'person_conf_thres': 0.7, 'person_iou_thres': 0.45,
          'kp_conf_thres': 0.5, 'kp_iou_thres': 0.45, 'conf_thres_kp_person': 0.2,
          'overwrite_tol': 25, 'kp_face': [0, 1, 2, 3, 4], 'use_kp_dets': True,
...
class kapao():  # Keypoint and Pose as Object
    def __init__(self, model_path):
        with open('class.names', 'rt') as fd:
    ...
    def postprocess(self, frame, outs, pad_size=None):
        ...
        for i in range(outs.shape[0]):
            ...
            confidence = scores[classId] * detection[4]  # class_scores * object_confidence
            if classId == 0:
        ...
        for i in person_indices:
            ...
            frame = self.drawPred(frame, person_classsIds[i], person_confidences,
    ...
    def detect(self, src_img):
        ...
        for i in range(self.nl):
            ...
            num_kp = (outs.shape - self.last_ind) // 2  # len([x1, y1, ..., xn, yn])  // 2
            kp = (4. * kp - 2.) * \
                 np.tile(np.repeat(self.anchor_grid[i], h * w, axis=0), (1, num_kp) + \
                         np.tile(np.tile(self.grid[i], (self.na, 1)) * int(self.stride[i]), (1, num_kp)))
