...
def non_max_suppression_kp(prediction, conf_thres=0.25, iou_thres=0.45, max_det=300, num_coords=24):
    """Runs Non-Maximum Suppression (NMS) on inference results
    prediction: (tensor)[batch, num_anchor_points, 41] = [B, 25200, 41], when num_head == 3, imgz==640
    41: [x,y,w,h,conf, c1,c2,..., c12, x1,y1,..., x12, y12]
    Returns:
         list of detections, on (n, 41) tensor per image [x, y, w, h, bbox_conf, kpt_vis_conf(k), kpt_xy(2k)] [41]
    """
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Box (center x, center y, width, height) to (x1, y1, x2, y2)
        box = xywh2xyxy(x[:, :4])
        # Compute conf [batch, 12]
        bbox_conf =  x[:, 4:5]
        kpt_vis_conf = x[:, 5:-num_coords]
        kpt_vis_conf *= bbox_conf   # conf = obj_conf * cls_conf

        kpt_xy = x[:, -num_coords:]
        # x: [x, y, w, h, bbox_conf, v1, v2, ...,vk, x1,y1, x2,y2, ..., xk,yk] [41]
        x = torch.cat((box, bbox_conf, kpt_vis_conf, kpt_xy), 1)
