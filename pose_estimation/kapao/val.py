    ...
    for si, (pd, kpd, origin) in enumerate(zip(person_dets, kp_dets, origins)):
        ...
        if num_det:
            ...
            # SINGLE-STAGE INFERENCE
                ...
                poses = poses.reshape((num_det, -data['num_coords'], 2))  # [num_det, num_kp, 2]
                # the confidence of keypoints which are predict with human bbox is default to 1.0
                poses = np.concatenate((poses, np.zeros((num_det, poses.shape[1], 1))), axis=-1)  # [num_det, num_kp, 3]
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='data/mixed-data-kp.yaml', help='dataset.yaml path')
