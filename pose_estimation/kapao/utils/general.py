...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    # Rescale coords (xyxy) from img1_shape (h, w) to img0_shape (h, w)
...
def non_max_suppression_kp(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, max_det=300, num_coords=34):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Detections matrix nx6 (xyxy, conf, cls)
        class_conf, class_id = x[:, 5:-num_coords].max(1, keepdim=True)
        kpt_xy = x[:, -num_coords:]
        # x: [x, y, w, h, bbox_conf, class_idx, x1, y1, x2, y2, ..., x17, y17] [40]
        x = torch.cat((box, class_conf, class_id.float(), kpt_xy), 1)[class_conf.view(-1) > conf_thres]
